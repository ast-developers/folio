...
class UserRepository implements UserRepositoryInterface
{
    ...
    {
        $users = User::with('userRole')->paginate(PAGINATE_LIMIT);
        return $users;
    }
    ...
    public function save($request, $id = null)
    {
        ...
        $values = ['name' => $request['name'], 'email' => $request['email'], 'remember_token' => $request['_token'], 'role_id' => $request['role']];
        $user->fill($values);
        $user->save();
    ...
    public function updateProfile($request)
    {
        ...
        $values = ['name' => $request['name'], 'email' => $request['email'], 'password' => (isset($request['password'])) ? $request['password'] : $user->password];
        $user->fill($values);
        $user->save();
    }
}
